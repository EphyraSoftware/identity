...
pub fn switch_profile(config: &mut LazyConfig, identity: Identity) -> anyhow::Result<()> {
    ...
    if !is_token_known(config, cargo_credentials.registry.token)? {
        let confirm = Confirm::new("The token in your Cargo credentials file is not known, overwrite anyway?");
        if !confirm.prompt()? {
            return Err(anyhow!("Will not overwrite credentials which are not known"));
        }
    }
...
fn is_token_known(config: &mut LazyConfig, token: String) -> anyhow::Result<bool> {
    for ic in &config.identity {
        ...
        if let Some(t) = ic.identity_for_service(CARGO_SERVICE)?.map(|i| i.token().cloned()).unwrap_or(None) {
            if t == token {
                return Ok(true);
            }
        }
